...
function(gklib_ipo_cache_entry output name value)
...
function(gklib_check_ipo result diagnostic)
  ...
  set(inputs "")
  gklib_ipo_cache_entry(inputs CMAKE_TRY_COMPILE_PLATFORM_VARIABLES
  ...
  foreach(name IN LISTS names)
    ...
    if(name MATCHES "^CMAKE_(C_COMPILER($|_ARG1$|_TARGET$|_EXTERNAL_TOOLCHAIN$|_AR$|_RANLIB$|_LAUNCHER$)|C_LINKER_LAUNCHER$|C_USING_LINKER_|C_LINK_MODE$|LINKER_TYPE$|C_STANDARD$|C_STANDARD_REQUIRED$|C_EXTENSIONS$|MSVC_RUNTIME_LIBRARY$|MSVC_DEBUG_INFORMATION_FORMAT$|SYSTEM_VERSION$|USER_MAKE_RULES_OVERRIDE($|_C$)|SYSROOT($|_)|OSX_|AR$|RANLIB$|RC_COMPILER$|RC_FLAGS$|MT$|LINKER$|TOOLCHAIN_FILE$|MAKE_PROGRAM$|FIND_ROOT_PATH($|_)|PREFIX_PATH$|LIBRARY_PATH$|INCLUDE_PATH$|C_STANDARD_LIBRARIES$)" OR
      ...
      gklib_ipo_cache_entry(inputs "${name}" "${${name}}")
    endif()
  endforeach()
  ...
  gklib_ipo_cache_entry(inputs CMAKE_BUILD_TYPE "${config}")
  if(CMAKE_CONFIGURATION_TYPES)
    gklib_ipo_cache_entry(inputs CMAKE_CONFIGURATION_TYPES "${config}")
  endif()
  ...
  gklib_ipo_cache_entry(inputs CMAKE_TRY_COMPILE_TARGET_TYPE STATIC_LIBRARY)
  gklib_ipo_cache_entry(inputs CMAKE_INTERPROCEDURAL_OPTIMIZATION OFF)
  gklib_ipo_cache_entry(inputs CMAKE_INTERPROCEDURAL_OPTIMIZATION_${upper} OFF)
  ...
  if(ARGC GREATER 4 AND TARGET "${ARGV4}")
    foreach(property COMPILE_OPTIONS LINK_OPTIONS POSITION_INDEPENDENT_CODE
      ...
      if(NOT value STREQUAL "value-NOTFOUND")
        gklib_ipo_cache_entry(inputs "PROBE_${property}" "${value}")
    ...
    if(NOT asan_namespace STREQUAL "asan_namespace-NOTFOUND")
      ...
      foreach(part PATHS WHOLE SYMBOLS)
        ...
        gklib_ipo_cache_entry(inputs "PROBE_ASAN_${part}" "${value}")
      endforeach()
      gklib_ipo_cache_entry(inputs PROBE_ASAN_MODULE
      ...
      gklib_ipo_cache_entry(inputs PROBE_ASAN_IMPLEMENTATION "${asan_implementation}")
    else()
      set(asan_namespace "")
    endif()
    ...
    if(NOT value STREQUAL "value-NOTFOUND")
      gklib_ipo_cache_entry(inputs PROBE_INTERFACE_LINK_OPTIONS "${value}")
    endif()
  endif()
  ...
  set(cache "GKLIB_IPO_CHECK_${key}")
  if(NOT DEFINED ${cache})
    string(SUBSTRING "${key}" 0 20 short_key)
    set(directory "${CMAKE_BINARY_DIR}/CMakeFiles/GKLIB-ipo/${short_key}")
    ...
    set(${cache} "${supported}" CACHE INTERNAL "GKLIB configured IPO link check")
    file(WRITE "${directory}/probe.log" "${reason}")
    set(${cache}_LOG "${directory}/probe.log" CACHE INTERNAL "GKLIB IPO check log")
  endif()
...
function(gklib_target_ipo target)
  ...
  foreach(config IN LISTS configs)
    ...
    set(required OFF)
    set(enabled OFF)
    if(NOT GKLIB_IPO STREQUAL "AUTO")
      set(enabled "${GKLIB_IPO}")
      set(required "${GKLIB_IPO}")
    ...
    if(enabled)
      gklib_check_ipo(supported diagnostic "${config}" "${type}" "${target}")
      if(NOT supported)
        if(required)
          message(FATAL_ERROR "Requested GKLIB IPO for configuration [${config}] is unavailable; see ${diagnostic}")
        endif()
        message(STATUS "GKLIB AUTO IPO disabled for configuration [${config}]; see ${diagnostic}")
        set(enabled OFF)
      endif()
    endif()
