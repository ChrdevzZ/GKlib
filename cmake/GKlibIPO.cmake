...
function(gklib_check_ipo result diagnostic)
  ...
  foreach(name IN LISTS names)
    ...
    if(name MATCHES "^CMAKE_C_(FLAGS($|_)|COMPILER($|_ID$|_VERSION$|_TARGET$|_FRONTEND_VARIANT$|_SIMULATE_ID$|_SIMULATE_VERSION$|_ARCHITECTURE_ID$|_AR$|_RANLIB$|_EXTERNAL_TOOLCHAIN$|_LINKER|_LAUNCHER$)|STANDARD$|STANDARD_REQUIRED$|EXTENSIONS$|LINKER_LAUNCHER$|LINK_OPTIONS_IPO$|COMPILE_OPTIONS_IPO$|ARCHIVE_)" OR
       ...
       name MATCHES "^GKLIB_(SANITIZERS|GPROF|NATIVE_OPTIMIZATION)$")
  ...
  if(NOT DEFINED ${cache})
    ...
    foreach(sanitizer IN LISTS GKLIB_SANITIZERS)
    ...
    set(${cache} "${supported}" CACHE INTERNAL "GKLIB IPO toolchain check")
    ...
    set(${cache}_LOG "${CMAKE_CURRENT_BINARY_DIR}/${cache}.log"
      CACHE INTERNAL "GKLIB IPO check log")
  endif()
...
function(gklib_target_ipo target)
  ...
  foreach(config IN LISTS configs)
    ...
    if(enabled)
      gklib_check_ipo(supported diagnostic)
      if(NOT supported)
        if(required)
          message(FATAL_ERROR "Requested GKLIB IPO is unavailable; see ${diagnostic}")
        endif()
        set(enabled OFF)
      endif()
    endif()
